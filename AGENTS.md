...
- Use RuneLite's `KeyManager` and `KeyListener` APIs for plugin keyboard input. Do not use `KeyboardFocusManager`, `KeyEventDispatcher`, global AWT or OS-level hooks, or injected keyboard events.
- Register listeners in `startUp()` and unregister them in `shutDown()`. Never capture or consume plugin hotkeys on the login screen.
- Prefer RuneLite's `HotkeyListener` for a single configurable or held `Keybind`; it provides exact matching, press/release state, typed-event consumption, and focus-loss recovery.
- Use a custom `KeyListener` for fixed or context-sensitive actions only when it gates the active context, consumes only handled events, and clears held state on focus loss.
- For multiple dynamic bindings, one central `KeyListener` may resolve enabled entries at press time, but it must retain the same repeat, release, focus-loss, typed-event, and login-state protections.
- Do not silently reuse a key combination for separate actions. Consumed events stop at the first matching action, so duplicate bindings make later actions appear broken.
